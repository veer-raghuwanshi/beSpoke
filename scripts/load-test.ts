...
if (process.env.LOAD_TEST !== 'true') throw new Error('Set LOAD_TEST=true to run the load test.');
...
try {
  const drop = await Drop.create({ item: runId, totalStock: stock, available: stock, liveAt: new Date(Date.now() - 1000), price: 10, maxPerUser: 1 });
  const users = Array.from({ length: virtualUsers }, (_, index) => `${runId}-user-${index}`);
  await Wallet.insertMany(users.map((userId) => ({ userId, balance: 100 })));
  ...
  const results = await Promise.all(users.map(async (userId) => {
    ...
    const response = await fetch(`${apiBaseUrl}/v1/drops/${drop._id}/claims`, {
      method: 'POST', headers: { 'content-type': 'application/json', 'x-user-id': userId, 'idempotency-key': `${runId}-${userId}` }, body: JSON.stringify({ quantity: 1 }),
    });
  ...
  console.log(JSON.stringify({ runId, virtualUsers, stock, successes, conflicts, finalAvailable: finalDrop?.available, activeHolds: holds, latencyMs: { min: Math.round(durations[0]), p50: Math.round(percentile(durations, .5)), p95: Math.round(percentile(durations, .95)), max: Math.round(durations.at(-1) ?? 0) }, passed }, null, 2));
