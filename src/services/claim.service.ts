...
export const claim = (dropId: string, userId: string, quantity: number, key: string) =>
  withTransaction(async (session) => {
    const existing = await Hold.findOne({ dropId: toObjectId(dropId), userId, idempotencyKey: key }).session(
      session
    );
    if (existing) {
